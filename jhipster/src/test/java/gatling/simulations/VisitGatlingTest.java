 ...
 */
public class VisitGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all visits").get("/api/visits").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new visit")
                        .post("/api/visits")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"date\": \"2020-01-01T00:00:00.000Z\"" + ", \"description\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created visit").get("${new_visit_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created visit").delete("${new_visit_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Visit entity").exec(scn);
